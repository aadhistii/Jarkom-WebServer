...
def tcp_server():
    ...
    while True:
        ...
        try:
            ...
            print("message from client: " + message) #Baris ini mencetak pesan yang diterima dari klien ke konsol. Pesan tersebut merupakan pesan HTTP yang dikirim oleh klien.
            request_method = message.split(' ')[0] #Baris ini memisahkan pesan HTTP menjadi beberapa bagian dengan menggunakan spasi (' ') sebagai pemisah. Kemudian, elemen pertama (indeks 0) dari hasil pemisahan tersebut disimpan dalam variabel request_method. Dalam konteks HTTP, elemen pertama biasanya merupakan metode permintaan, seperti "GET", "POST", dll.
            request_path = message.split(' ')[1] #Baris ini melakukan hal yang sama seperti baris sebelumnya, tetapi kali ini elemen kedua (indeks 1) dari hasil pemisahan disimpan dalam variabel request_path. Dalam konteks HTTP, elemen kedua umumnya merupakan jalur permintaan, seperti "/index.html"
            ...
            if request_method == 'GET':
                if request_path == '/':
                    ...
                    try:
                        ...
                        elif request_path.endswith('.woff2'): #cek file dalam tipe konten woff2 untuk bentuk tulisam
...
def handle_request(file_path):
    ...
    content_length_header = "Content-Length: " + str(len(file_content)) + "\r\n" #berisi header "Content-Length" dari respon HTTP. Nilai len(file_content) adalah panjang (jumlah byte) dari file_content, yang merupakan isi dari file yang akan dikirimkan ke klien.
    response = response_header + content_type_header + content_length_header + "\r\n" + file_content.decode() # Baris ini menggabungkan semua elemen respon HTTP menjadi satu string yang lengkap ditambah dengan hasil dari pengubahan file_content dari bentuk byte menjadi string menggunakan metode decode()
